    ...
    <section id="portfolio" className="section-surface-alt wm-section">
      <div className="wm-container">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {portfolioItems.slice(0, 4).map((item, index) => (
            <Reveal key={item.title} delay={index * 70}>
              <Link href="/portfolio">
                <SpotlightCard className="media-card group relative min-h-[360px] overflow-hidden rounded-[8px] border border-white/10 bg-wm-panelSoft">
                  <Image
                    src={item.image}
        ...
        <div className="mt-8">
          <Link
            href="/portfolio"
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-[8px] border border-white/10 bg-white/[0.04] px-5 py-3 text-sm font-black uppercase tracking-[0.08em] text-white transition hover:border-wm-red/60 hover:bg-white/10"
          >
